...
function renderTemplateSections(
  ...
  // EXTRA IMAGE Full width (contain)
  if (template.extraImage) {
    const extraImg = resolveMediaUrl(template.extraImage)
    if (extraImg) {
      parts.push(`
        <div style="text-align:center;margin:0 0 18px;">
          <img src="${extraImg}" width="520" alt="Extra Image"
            style="display:block;border:0;max-width:520px;width:100%;height:auto;margin:0 auto;">
        </div>
      `)
    }
  ...
  if (orderId) {
    //need to add a link to the order cancel api route
    parts.push(
      `<p style="margin:0 0 14px;font-family:'Merriweather',Georgia,serif;font-size:14px;line-height:1.6;color:#B9ACC8;">
          ...
          <td style="background:linear-gradient(135deg,#1A0F26 0%,#3C1D1D 100%);padding:20px;">
            <table role="presentation" width="100%">
              <tr>
                ...
                <td align="right" valign="middle" style="width:50%; font-family:'Merriweather',Georgia,serif;font-size:17px;line-height:1.4;color:#E8C468;font-weight:600;text-align:right;">
                  ${description}
                </td>
              </tr>
            </table>
